...
public class AsyncGoalExecutor {
    ...
    public AsyncGoalExecutor(AsyncGoalThread thread, ServerLevel world) {
        this.world = world;
        this.queue = new SpscIntQueue(GlobalConfiguration.get().asyncTargetFinding.queueSize);
        this.wake = new SpscIntQueue(GlobalConfiguration.get().asyncTargetFinding.queueSize);
    ...
    public final void midTick() {
        if (GlobalConfiguration.get().asyncTargetFinding.threshold <= 0L || (midTickCount % GlobalConfiguration.get().asyncTargetFinding.threshold) == 0L) {
            batchSubmit();
        }
